    ...
    <div class="card-home-wrapper">
      ...
      >
        ...
        >
          ...
          <div style="font-size: 15px; margin: 5px">
            TOTAL N° PROJECTS: {{ projects.length }} projects.
          </div>
        </div>
        <div
          class="explanations"
        ...
        >
          ...
          <div style="font-size: 15px; margin: 5px">
            TOTAL N° ENTERPRISES: {{ enterprises.length }} enterprises.
          </div>
        </div>
      </card>
      <card
      ...
      >
        <div
          ...
          style="cursor: pointer; margin: 25px 5px 20px 20px; font-size: 20px"
        >
        ...
        <div v-if="projectCreationTrigger">
          ...
          <button class="simple-button" @click="toggleProjectCreation">
            CANCEL
          </button>
        </div>
      </card>
      ...
      >
        ...
        <div v-if="enterpriseCreationTrigger">
          ...
          <button class="simple-button" @click="toggleEnterpriseCreation">
            CANCEL
          </button>
        </div>
      </card>
  ...
  methods: {
    ...
    async createProject() {
      ...
      await contract.methods
        .createProject(this.projectName, 0, this.projectTokensBalance)
        .send()
      await this.updateAccount()
    ...
    async donateToProject(project: any) {
      const { contract } = this
      await contract.methods
        .supportProject(project.id, this.projectDonation)
        .send()
      await this.updateAccount()
      this.projectCreationTrigger = false
